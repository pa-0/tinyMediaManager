...
/**
 * the class {@link TvShowDiscArtSizeFilter} provides a filter for TV discart width
 * 
 * @author Manuel Laggner
 */
public class TvShowDiscArtSizeFilter extends AbstractNumberTvShowUIFilter {
  public TvShowDiscArtSizeFilter() {
    super();
  ...
  protected boolean accept(TvShow tvShow, List<TvShowEpisode> episodes, boolean invert) {
    try {
      ...
      // TV show
      boolean foundShow = matchInt(tvShow.getArtworkDimension(MediaFileType.DISC).width);
      ...
      for (TvShowEpisode episode : episodes) {
        boolean foundEpisode = matchInt(episode.getArtworkDimension(MediaFileType.DISC).width);
  ...
  protected JLabel createLabel() {
    return new TmmLabel(TmmResourceBundle.getString("filter.discart.width"));
  }

  @Override
  public String getId() {
    return "tvShowDiscArtSize";
  }
}

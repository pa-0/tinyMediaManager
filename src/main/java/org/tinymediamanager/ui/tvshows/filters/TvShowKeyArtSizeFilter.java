...
/**
 * the class {@link TvShowKeyArtSizeFilter} provides a filter for TV keyart height
 * 
 * @author Manuel Laggner
 */
public class TvShowKeyArtSizeFilter extends AbstractNumberTvShowUIFilter {
  public TvShowKeyArtSizeFilter() {
    super();
  ...
  protected boolean accept(TvShow tvShow, List<TvShowEpisode> episodes, boolean invert) {
    try {
      ...
      // TV show
      boolean foundShow = matchInt(tvShow.getArtworkDimension(MediaFileType.KEYART).height);
      ...
      for (TvShowEpisode episode : episodes) {
        boolean foundEpisode = matchInt(episode.getArtworkDimension(MediaFileType.KEYART).height);
